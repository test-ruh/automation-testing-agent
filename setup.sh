#!/usr/bin/env bash
# =============================================================================
# One-command setup for Automation Testing Agent
...
AGENT_NAME="Automation Testing Agent"
...
phase_env() {
  ...
  "APP_URL|App URL|Target application URL to open for the test run.|Open https://app.rapidinnovation.dev/my-agents in your browser\nCopy the full page URL from the address bar\nPaste it into the App URL field above"
  "APP_USERNAME|Username or email|Username or email used to sign in to the app.|Open the credentials provided for this test run\nFind the username or email value\nCopy the username or email and paste it into the field above"
  "APP_PASSWORD|Password|Password used to sign in to the app.|Open the credentials provided for this test run\nFind the password value\nCopy the password and paste it into the field above"
  "FEATURE_NAME|Feature name|Name of the feature to test in the authenticated app.|Open the test request details provided for this run\nFind the feature name value\nCopy the feature name and paste it into the field above"
  "WEB_BROWSER|Web browser|Browser channel used to interact with the application UI.|Open the browser runtime or browser tab used by the agent\nWatch the app pages load and the actions run in the UI\nUse this channel to inspect page behavior during the test"
  "RESULTS_REPORT|Results report|Final report channel for pass/fail output, bugs, and evidence references.|Open the final execution summary produced by the agent\nReview the step outcomes, bug reports, and evidence links\nUse this channel to share the test result with the requester"
  )
...
phase_db() {
  ...
  if [[ -n "${PG_CONNECTION_STRING:-}" ]]; then
    ...
    if ${local_pg}; then
      ...
      warn "     Then set in .env: PG_CONNECTION_STRING=postgresql://postgres:<choose-a-password>@localhost:5432/automation-testing-agent"
      exit 1
    fi
